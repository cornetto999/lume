-- Prefer compatible vibe, language, country and topic matches in the atomic path.
...
DECLARE
  ...
  v_cooldown_expires_at TIMESTAMPTZ := now() + interval '10 minutes';
BEGIN
  ...
  WHERE mq.status = 'searching'
    ...
    AND (
      COALESCE(current_member.preferences->>'language', 'Any') = 'Any'
